...
**Version:** 2.0  
**Status:** IN PROGRESS  
**Course:** DDM501 — Lab 3 (5%) + Lab 4 (15%)  
**Tech Stack:** LangGraph · LangChain · gpt-4o-mini · ChromaDB · Tavily · Gradio · Langfuse (self-hosted Docker)

---

## Lab Requirements Coverage

### Lab 3 (5%) — Multi-Agent Chatbot với LangGraph & Gradio
| Yêu cầu | Task | Status |
|---|---|---|
| StateGraph + nodes + edges | T5.2 `app/graph.py` | 🔲 |
| Chat history trong State (`messages`) | T0.3 | ✅ done |
| Memory checkpoints (MemorySaver) | T5.2 | 🔲 |
| Semantic Router LLM-based (JSON output) | T4.4 + T5.1 | 🔲 |
| General Agent (chit-chat) | T4.1 | 🔲 |
| RAG Agent (knowledge retrieval) | T4.2 | 🔲 |
| Gradio UI với route metadata indicator | T6.1 | 🔲 |
| ≥6 pytest tests cho routing | T7.0 | 🔲 |
| README.md với architecture + setup | T8.0 | 🔲 |

### Lab 4 (15%) — LLM Observability với self-hosted Langfuse
| Yêu cầu | Task | Status |
|---|---|---|
| `docker-compose.yml` (Langfuse + PostgreSQL) | T0.5 | ✅ done |
| `app/observability.py` → self-hosted localhost:3000 | T0.5 | ✅ done |
| UUID `trace_id` per request | T0.5 | ✅ done |
| Metadata: `user_id`, `session_id`, `tags` | T0.5 + T5.2 | ✅/🔲 |
| 👍/👎 feedback buttons trong Gradio | T6.1 | 🔲 |
| `submit_feedback()` → Langfuse score | T0.5 | ✅ done |
| `tests/test_observability.py` (≥4 tests) | T7.3 | 🔲 |
| README Docker startup instructions | T8.0 | 🔲 |
...
Wave 7:             T5.2 ◄── T4.1+T4.2+T4.3+T4.4+T0.5
                     │
Wave 8 (parallel):  T6.1  T7.0  T7.1  T7.2  T7.3  T8.0
```
...
| Wave | Tasks (song song) | Điều kiện | Lab |
|---|---|---|---|
| 0 | ~~T0.1~~, ~~T0.2~~, ~~T0.3~~, ~~T0.4~~, ~~T0.5~~, T1.1, T1.2 | ✅ Xong (trừ T1.x) | — |
| 2 | T2.1, T2.2, T2.3, T2.4, T2.5 | T0.4 + T1.1 done | — |
| 3 | T3.1 | T2.5 + T1.1 done | — |
| 5 | T4.1, T4.2, T4.3, T4.4 | Deps tương ứng | Lab 3 |
| 6 | T5.1 | T4.4 done | Lab 3 |
| 7 | T5.2 | T4.x + T5.1 + T0.5 done | Lab 3+4 |
| 8 | T6.1, T7.0, T7.1, T7.2, T7.3, T8.0 | T5.2 done | Lab 3+4 |

### Tasks mới bổ sung cho Lab 3 & Lab 4

**T7.0 — `tests/test_routing.py`** (Lab 3 — bắt buộc ≥6 tests)
```python
@pytest.mark.parametrize("query,expected", [
    ("Xin chào!", "general"),
    ("Cho tôi tips về visa Nhật", "general"),
    ("Thời tiết ở đây thế nào?", "general"),
    ("Lên kế hoạch 3 ngày Đà Nẵng ngân sách 8 triệu", "travel"),
    ("Tôi muốn đi Phú Quốc 5 ngày", "travel"),
    ("Đặt lịch trình Hà Nội 2 ngày", "travel"),
])
def test_router_classification(query, expected): ...
```

**T7.3 — `tests/test_observability.py`** (Lab 4 — bắt buộc ≥4 tests)
```python
def test_trace_id_is_valid_uuid()          # UUID format validation
def test_feedback_handles_empty_trace_id() # Graceful None trace_id
def test_handler_none_without_keys()       # Graceful khi thiếu keys
def test_host_validation()                 # LANGFUSE_HOST = localhost:3000
```

**T5.2 update** — Thêm vào `app/graph.py`:
- `MemorySaver` checkpoint (Lab 3: persistent chat history)
- Inject Langfuse `CallbackHandler` với `trace_id` UUID (Lab 4)
- Pass `session_id`, `user_id`, `tags` vào handler

**T6.1 update** — Thêm vào `ui/gradio.py`:
- Route metadata mỗi response: `🤖 [General Agent]` / `✈️ [Travel Planning]` (Lab 3)
- `gr.State` lưu `trace_id` và `session_id` (Lab 4)
- 👍/👎 buttons → `submit_feedback(trace_id, 1.0/0.0)` (Lab 4)

**T8.0 — `README.md`** (Lab 3+4)
- Architecture diagram (Mermaid)
- Setup: venv, pip install, .env config
- Docker: `docker-compose up -d` → Langfuse tại localhost:3000
- Run: `python main.py`
- Sample query logs với state changes + route indicator

**Ước tính còn lại:** ~12–16 giờ làm việc thực tế.
